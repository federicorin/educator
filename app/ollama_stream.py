...
class GroqClientWrapper:
    def __init__(self, api_key: Optional[str] = None):  # Ã¢Å“â€¦ Corregido: __init__
        if Groq is None:
    ...
    def search(self, prompt: str, top_k: int = 5) -> List[Dict]:
        ...
        ai_description = os.getenv("AI_DESCRIPTION", "Soy un asistente ÃƒÂºtil")
        ai_style = os.getenv("AI_RESPONDER_STYLE", "claro y conciso")
        
        system_prompt = f"Eres {ai_name}"
        if ai_institution:
            system_prompt += f" de {ai_institution}"
        system_prompt += f". {ai_description}. Respondes de manera {ai_style} y siempre en espaÃƒÂ±ol."
        ...
        resp = self.client.chat.completions.create(
            ...
            max_tokens=8192,
            temperature=0.7,  # Un poco mÃƒÂ¡s de creatividad
        )
...
def ollama_run_for_kb(query: str, namespace: str = "default", top_k: int = 5):
    ...
    try:
        client = GroqClientWrapper()  # Ã¢Å“â€¦ Ahora buscarÃƒÂ¡ GROQ_API_KEY automÃƒÂ¡ticamente
